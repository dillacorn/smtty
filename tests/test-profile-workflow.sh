...
[[ "$guided_block" == *'if choose_drm_output; then step=3; else step=1; fi'* ]]
[[ "$guided_block" == *'if choose_pipewire_latency; then step=8; else step=6; fi'* ]]
[[ "$guided_block" == *'if choose_pre_post_commands; then step=12; else step=10; fi'* ]]
